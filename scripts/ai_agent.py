...
def setup_clients():
    ...
    if not os.path.exists(json_key_path):
        # Fallback: prova a cercarlo nella cartella superiore se non è in scripts
        json_key_path = os.path.join(os.path.dirname(
            __file__), '..', 'service_account.json')

    if not os.path.exists(json_key_path):
        raise FileNotFoundError(
            "ERRORE: Non trovo il file 'service_account.json'. Scaricalo dalla Cloud Console e mettilo nella cartella scripts!")

    creds = Credentials.from_service_account_file(json_key_path, scopes=scope)
    gc = gspread.authorize(creds)
    # -------------------------------------------------------------------------------

    # Le altre chiavi le prende dal file .env come volevi
