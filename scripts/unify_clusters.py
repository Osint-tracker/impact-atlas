...
def unify_clusters(incremental=False, recent_days=None):
    ...
    start_date = robust_parse_date(min_s) or (datetime.now(UTC) - timedelta(days=60))
    end_date = robust_parse_date(max_s) or datetime.now(UTC)
    ...
    # Build WHERE clause based on mode (clause text is static; only the
    # cutoff value is bound as a parameter).
    ...
    if recent_days:
        cutoff = (datetime.now(UTC) - timedelta(days=recent_days)).strftime('%Y%m%d')
        where_clauses.append("date_published >= ?")
        bind_params.append(cutoff)
