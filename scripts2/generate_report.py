...
class C:
    ...
    DB       = DIR.parent / 'war_tracker_v2' / 'data' / 'raw_events.db'
    FONTS    = DIR / 'fonts'
    OUT      = DIR / 'reports'
...
def calculate_time_period(date_str, latest_dt):
    try:
        dt = datetime.datetime.strptime(date_str[:10], "%Y-%m-%d")
        delta = (latest_dt.date() - dt.date()).days
        if delta == 0:
            return 'current_24h'
        elif delta == 1:
            return 'prev_24h'
    ...
    def fetch(self, days=7):
        ...
        for r in rows:
            ...
            events.append({
                ...
                'sector':   r['operational_sector'] or 'UNKNOWN_SECTOR',
        ...
        # Sector specific analysis components
        latest_date_str = max([e['date'] for e in events]) if events else "2000-01-01"
        try:
            latest_dt = datetime.datetime.strptime(latest_date_str[:10], "%Y-%m-%d")
        except:
        ...
        for e in events:
            s_name = e['sector']
            period = calculate_time_period(e['date'], latest_dt)
            
            if s_name not in tie_current:
                tie_current[s_name] = 0; tie_prev[s_name] = 0; count_current[s_name] = 0; count_prev[s_name] = 0
                sector_taxonomy[s_name] = Counter()

            sector_taxonomy[s_name][e['tgt_cat']] += 1

            if period == 'current_24h':
                tie_current[s_name] += e['tie']
                count_current[s_name] += 1
            elif period == 'prev_24h':
                tie_prev[s_name] += e['tie']
                count_prev[s_name] += 1
                
            # Dynamic alert rules
            if e['tgt_cat'] in ['MANOEUVRE', 'SHAPING_OFFENSIVE'] and s_name in ['REAR_AREA_UA', 'Kyiv', 'Lviv', 'Odesa']:
                top_critical_alerts.append(f"Red Alert: {e['tgt_cat']} detected in non-combat sector '{s_name}'.")

        # Shifts compilation
        for s in tie_current.keys():
            delta = tie_current[s] - tie_prev[s]
            if abs(delta) > 50:  # Meaningful shift
                shift_type = "escalation" if delta > 0 else "de-escalation"
                sector_shifts[s] = f"{delta:+.1f} T.I.E."
            
            # Critical Sector Alert rule
            if count_current[s] > 0 and count_prev[s] > 0:
                avg_curr = tie_current[s] / count_current[s]
                avg_prev = tie_prev[s] / count_prev[s]
